...
const ProjectDetails = () => {
  ...
  useEffect(() => {
    ...
    // First try to find the project in the imported projects data
    ...
    if (selectedProject) {
      ...
      // Fallback to checking localStorage if not found in imported data
      const storedProjects = JSON.parse(localStorage.getItem("projects")) || [];
      const storedProject = storedProjects.find((p) => String(p.id) === id);
      
      if (storedProject) {
        const enhancedProject = {
          ...storedProject,
          Features: storedProject.Features || [],
          TechStack: storedProject.TechStack || [],
          Github: storedProject.Github || 'https://github.com/EkiZR',
        };
        setProject(enhancedProject);
      }
      setIsLoading(false);
    ...
    return (
      <div className="min-h-screen bg-[#030014] flex items-center justify-center">
        <div className="text-center space-y-6 animate-fadeIn">
          ...
          <button
            onClick={() => navigate(-1)}
            className="inline-flex items-center space-x-2 px-5 py-3 bg-white/5 backdrop-blur-xl rounded-xl text-white/90 hover:bg-white/10 transition-all duration-300 border border-white/10 hover:border-white/20"
          >
